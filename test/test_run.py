...
def test_verify_seg():
    ...
    seg1 = NaiveTrajectorySeg(1, 0, points1)
    res = verify_seg(region, seg1, 3)
    assert len(res) == 1
    assert res == [RunTimeTrajectorySeg(1, 11, 3)]
    res = verify_seg(region, seg1, 2)
    assert len(res) == 4
    assert [seg.begin for seg in res] == [1, 5, 8, 11]

    seg2 = NaiveTrajectorySeg(1, 0, points1[1:-1])
...
def test_candidate_verified_queue():
    ...
    cand = [[NaiveTrajectorySeg(i, j*10, obj_ps) for j, obj_ps in enumerate(ind)] for i, ind in enumerate(all_points)]
